...
- **설계서 참조 주석 금지**: `docs/` 내 문서를 **제외한** 어떤 곳(코드 주석, docstring, 커밋 메시지, 마이그레이션 등)에도 설계서를 가리키는 주석·코멘트를 넣지 않는다. `§3.3`·`설계 §0.3`·`models-integration-design.md 참조` 같은 표현 금지. 코드 주석은 **그 자체로 자족적**이어야 하며, 설계 근거가 필요하면 절 번호를 인용하지 말고 **내용을 직접 서술**한다.

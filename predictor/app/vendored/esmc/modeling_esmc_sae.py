...
from transformers.modeling_outputs import ModelOutput  # type: ignore[import]
from transformers.modeling_utils import PreTrainedModel  # type: ignore[import]
from transformers.utils import auto_docstring  # type: ignore[import]
...
class _ESMCSAELayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **_kwargs: object) -> ESMCSAEOutput:
        ...
        feature_magnitudes = torch.zeros_like(preactivations).scatter(-1, topk.indices, topk.values)
    ...
    def get_sae_output(self, layer_states: torch.Tensor, token_mask: torch.Tensor) -> ESMCSAEOutput:
...
class ESMCSAEModel(ESMCSAEPreTrainedModel):
    ...
    def initialize_layers(
        ...
        *,
        ...
        assert self._snapshot_dir is not None, (
            "ESMCSAEModel has no snapshot directory — call " "from_pretrained first, or set _snapshot_dir manually."
        )
        if device is None:
        ...
        for layer_idx in layers:
            ...
            if layer_idx not in available:
                raise KeyError(f"Layer {layer_idx} is not in this repo. " f"available_layers={sorted(available)}")
    ...
    def forward(self, x: torch.Tensor, layer: int | None = None, **kwargs: object) -> ESMCSAEOutput:
        if layer is None:
    ...
    def save_pretrained(self, save_directory: str | os.PathLike, *args, **kwargs) -> None:  # type: ignore[override]
        ...
        for key, layer in self.layers.items():
            ...
            save_file(
                {k: v.detach().cpu().contiguous() for k, v in layer.state_dict().items()},
                str(shard),
            )


def _resolve_snapshot_dir(pretrained_model_name_or_path: str | os.PathLike, kwargs: dict) -> str:

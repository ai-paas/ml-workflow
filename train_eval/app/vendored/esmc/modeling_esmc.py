...
from transformers.modeling_outputs import (  # type: ignore[import]
    MaskedLMOutput,
    ModelOutput,
...
from transformers.utils import (  # type: ignore[import]
    auto_docstring,
    can_return_tuple,
    ...
    def _update_cos_sin_cache(self, seqlen: int, device=None, dtype=None):
        ...
        if (
            ...
            if self.pos_idx_in_fp32:
                ...
                inv_freq = self.inv_freq.to(torch.float32) if self.inv_freq.dtype != torch.float32 else self.inv_freq
            else:
                t = (
            ...
            if self.scale is None:
                ...
                power = (
                    torch.arange(seqlen, dtype=_scale.dtype, device=_scale.device) - seqlen // 2
...
def _make_attn_layernorm_qkv(d_model: int, bias: bool) -> nn.Module:
    ...
    assert not bias, "ESMC was trained with bias=False; bias=True not supported"
    if _te_available:
        return te.LayerNormLinear(d_model, d_model * 3, bias=bias, init_method=None)  # type: ignore[union-attr]
...
def _make_attn_out_proj(d_model: int, bias: bool) -> nn.Module:
    ...
    if _te_available:
        return te.Linear(d_model, d_model, bias=bias, init_method=None)  # type: ignore[union-attr]
    return nn.Linear(d_model, d_model, bias=bias)


def _drop_te_extra_state(module: nn.Module, state_dict: dict, prefix: str, local_metadata: dict) -> None:
...
def _scaled_dot_product_attention(
    ...
    *,
    ...
    if seq_id is None and _flash_attn_available and q.dtype in (torch.float16, torch.bfloat16):
        b, s, _ = q.shape
        q4 = q.view(b, s, n_heads, d_head)
        k4 = k.view(b, s, n_heads, d_head)
        v4 = v.view(b, s, n_heads, d_head)
        context = flash_attn_func(q4, k4, v4, dropout_p=0.0, softmax_scale=d_head**-0.5)  # type: ignore[misc]
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        context = _scaled_dot_product_attention(q, k, v, n_heads=self.n_heads, d_head=self.d_head, seq_id=seq_id)
...
class _FlashMultiHeadAttention(MultiHeadAttention):
    ...
    def __init__(self, d_model: int, n_heads: int, bias: bool = False, qk_layernorm: bool = True):
        super().__init__(d_model=d_model, n_heads=n_heads, bias=bias, qk_layernorm=qk_layernorm)
...
class ESMCModel(ESMCPreTrainedModel):
    ...
    def __init__(self, config: ESMCConfig):
        ...
        self._use_flash_attn = _flash_attn_available and config._attn_implementation == "flash_attention_2"
    ...
    def add_sae_models(self, sae_models: list[_ESMCSAELayer]) -> None:
        ...
        for layer in sae_models:
            assert isinstance(layer, _ESMCSAELayer), (
                f"Expected an SAE layer (model.layers['<idx>']), got " f"{type(layer).__name__}."
            )
